...
SwitchControl::SwitchControl(pcl::visualization::PCLVisualizer& viewer, std::vector<std::string> states, std::string keyBackward, std::string keyForward, int place,
	...
	if (keyForward == "Right") {
		maxY= 65;
		viewer.addText("[ Up | Down | Left | Right ]", 100, maxY, 1, 0, 0, id2Str + "keys");
		propertyNames = std::vector<std::string>{ "age", "weight", "gender" };
		i = 0;
		for (auto& state : propertyNames) {
			viewer.addText((i == 0 ? ">" : " ") + state + ": " + std::to_string(m_props[i]), 100, maxY - (i + 1) * c_textHeight, 0, 1, 0, id2Str + "option " + std::to_string(i));
			i++;
		}
	}
	else {
		maxY= (m_numStates + 1+place) * c_textHeight + s_currentY;
		std::cout << maxY << endl;
    ...
    viewer.registerKeyboardCallback([this, states, idStr, id2Str, maxY, propertyNames, keyBackward, keyForward, &viewer, callee](const pcl::visualization::KeyboardEvent keyEvent) {
        ...
        if (keyForward == "Right") {
			...
			viewer.updateText(" " + propertyNames[m_selectedProp] + ": " + std::to_string(m_props[m_selectedProp]), 100, maxY - (m_selectedProp + 1) * c_textHeight, 0, 1, 0, id2Str + "option " + std::to_string(m_selectedProp));
